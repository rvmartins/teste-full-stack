...
<div class="page-header">
    <h1>
        <i class="fa fa-stethoscope"></i> Lista de Especialidades Médicas
        <small>Gerenciar especialidades do sistema</small>
    </h1>
</div>
...
<div class="row">
    ...
    <div class="col-md-3">
        <select class="form-control" id="ativaFilter">
            <option value="">Todas as especialidades</option>
            <option value="1" {{ request('ativa') == '1' ? 'selected' : '' }}>Apenas ativas</option>
            <option value="0" {{ request('ativa') == '0' ? 'selected' : '' }}>Apenas inativas</option>
        </select>
    </div>
    
    <div class="col-md-3">
        <a class="btn btn-success btn-block" href="{{ route('especialidades.create') }}">
            <i class="fa fa-plus"></i> Nova Especialidade
        </a>
    </div>
</div>
...
<div id="tableContainer">
        ...
        <div class="table-responsive">
            <table class="table table-bordered table-striped">
                ...
                <tbody>
                    ...
                    <tr>
                        <td><strong>{{ $especialidade->nome }}</strong></td>
                        <td>
                            @if($especialidade->descricao)
                                {{ str_limit($especialidade->descricao, 50) }}
                            @else
                        ...
                        <td>
                            <span class="badge">{{ $especialidade->entidades ? $especialidade->entidades->count() : 0 }}</span>
                        </td>
                        <td>{{ $especialidade->created_at->format('d/m/Y H:i') }}</td>
                        <td>
                            <div class="btn-group btn-group-xs">
                                <a class="btn btn-info" href="{{ route('especialidades.show', $especialidade->id) }}" title="Visualizar">
                                    <i class="fa fa-eye"></i>
                                </a>
                                <a class="btn btn-primary" href="{{ route('especialidades.edit', $especialidade->id) }}" title="Editar">
    ...
    @else
        <div class="text-center" style="padding: 40px;">
            ...
            <p class="text-muted">Não há especialidades cadastradas ou que atendam aos filtros aplicados.</p>
            <a href="{{ route('especialidades.create') }}" class="btn btn-primary">
                <i class="fa fa-plus"></i> Cadastrar primeira especialidade
            </a>
        </div>
    @endif
</div>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p>Tem certeza que deseja excluir a especialidade <strong id="entityName"></strong>?</p>
...
<script>
    $(document).ready(function() {
        ...
        function performSearch() {
            ...
            url.searchParams.set('ativa', ativa);
