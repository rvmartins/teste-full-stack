...
<div class="page-header">
    <h1>
        <i class="fa fa-edit"></i> Editar Clínica
        <small>{{ $entidade->nome_fantasia }}</small>
    </h1>
</div>

<div class="row">
    <div class="col-md-8 col-md-offset-2">
        <form method="POST" action="{{ route('entidades.update', $entidade->id) }}" id="entityForm" class="no-loading">
            {{ csrf_field() }}
            {{ method_field('PUT') }}
                ...
                <div class="panel-body">
                    <div class="form-group">
                        ...
                        <input type="text" 
                               ...
                               id="razao_social" 
                               name="razao_social" 
                               value="{{ old('razao_social', $entidade->razao_social) }}" 
                               required
                               maxlength="255">
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" 
                               ...
                               id="cnpj" 
                               name="cnpj" 
                               value="{{ old('cnpj', $entidade->cnpj_formatado) }}" 
                               required
                ...
                <div class="panel-body">
                    <div class="form-group">
                        ...
                        <select class="form-control" id="regional" name="regional" required>
                            ...
                            @foreach($regionais as $key => $value)
                                <option value="{{ $key }}" {{ old('regional', $entidade->regional) == $key ? 'selected' : '' }}>
                                    {{ $value }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <input type="date" 
                               ...
                               value="{{ old('data_inauguracao', $entidade->data_inauguracao->format('Y-m-d')) }}" 
                               required>
                    </div>
                        ...
                        <div class="checkbox">
                            <label>
                                <input type="checkbox" 
                                       name="ativa" 
                                       value="1" 
                                       {{ old('ativa', $entidade->ativa) ? 'checked' : '' }}> 
                        ...
                        <select class="form-control" id="especialidades" name="especialidades[]" multiple required>
                            @foreach($especialidades as $especialidade)
                                <option value="{{ $especialidade->id }}" 
                                        {{ in_array($especialidade->id, old('especialidades', $especialidadesSelecionadas)) ? 'selected' : '' }}>
                                    {{ $especialidade->nome }}
                                </option>
                            @endforeach
                        </select>
            ...
            <div class="form-group">
                <button type="submit" class="btn btn-primary btn-lg" id="submitBtn">
                    <i class="fa fa-save"></i> Atualizar Clínica
                </button>
                <a class="btn btn-default btn-lg" href="{{ route('entidades.index') }}">
                    <i class="fa fa-arrow-left"></i> Cancelar
                </a>
                <a class="btn btn-info btn-lg" href="{{ route('entidades.show', $entidade->id) }}">
